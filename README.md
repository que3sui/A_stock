...
## 主要结果 (2024-2025 OOS 565 交易日, 真实约束)

| 模型                   | 类型       | RankIC              | 年化 ICIR | 年化收益 | 夏普     | 最大回撤   |
| ---------------------- | ---------- | ------------------- | --------- | -------- | -------- | ---------- |
| HS300 基准             | -          | -                   | -         | 19.6%    | 1.08     | -15.7%     |
| LightGBM               | 树         | 0.0459              | 5.21      | 45.8%    | **2.21** | -15.5%     |
| MLP                    | DL 基线    | 见 mlp_metrics.json |           |          |          |            |
| GRU+Att                | 时序 DL    | 0.0443              | 4.42      | 21.6%    | 0.80     | -31.6%     |
| **MASTER v1**          | **主模型** | 0.0572              | 6.08      | 42.8%    | 2.12     | **-11.1%** |
| MASTER v2 (加深)       | 反例       | 0.0476              | 6.02      | 29.7%    | 1.12     | -21.4%     |
| MASTER v3 (multi-seed) | 减方差     | **0.0580**          | **6.98**  | 41.5%    | 1.64     | -20.3%     |
| Ensemble               | 加权       | 0.0552              | 5.51      | 43.7%    | 2.19     | -10.7%     |

**主模型选 MASTER v1**: 满足深度学习作业要求 + 较好的回撤控制 + 显式横截面建模 + market guidance 模块化设计。
...
| 训练/验证/测试 | 2016-2022 / 2023 / 2024-2025                          | 严格按时间, OOS 565 天足够稳健                        |
...
> | 姓名 | 学号    | 主要分工                                        |
> | ---- | ------- | ----------------------------------------------- |
> | XXX  | SAxxxxx | 数据预处理 + 特征工程 + 文档撰写                |
> | XXX  | SAxxxxx | 模型设计与训练 (MASTER/GRU/MLP/LGBM) + 消融实验 |
> | XXX  | SAxxxxx | 回测引擎 + 报告生成 + 模拟交易执行              |
